...
exports.queryUser = function(userId, callback) {
    mysql.queryOne("select * from userBlog where id = ?", [ userId ], function(err, user) {
        callback(err, user);
    });
};
...
exports.queryUsers = function(userIds, callback){
    mysql.query('select * from userBlog where id in ('+userIds.join(',')+')',function(err, users){
        callback(err, users);
    });
};
...
exports.queryAllUsers = function(callback){
    mysql.query('select * from userBlog',function(err, users){
        callback(err, users);
    });
};
...
exports.queryUserByLoginName = function(loginname, callback) {
    mysql.queryOne('select * from userBlog where loginname=?', [ loginname ], function(err, user) {
        callback(err, user);
    });
...
exports.queryUsersByEmailOrLoginName = function(loginname, email, callback){
    mysql.query('select * from userBlog where loginname=? or email=?', [ loginname, email ], function(err, users) {
        callback(err, users);
    });
};
...
exports.queryAllFollowers = function(userId, callback) {
    mysql.query('select * from userBlog where id in(select user_id from follow where following_id = ?)', [ userId ], function(err, users) {
        callback(err, users);
    });
};
...
exports.queryAllFollowings = function(userId, callback) {
    mysql.query('select * from userBlog where id in(select following_id from follow where user_id = ?)', [ userId ], function(err, users) {
        callback(err, users);
    });
};
...
exports.saveUser = function(loginname, email, pass, createAt, avatarUrl, callback){
    mysql.update('insert into userBlog(loginname,email,pwd,create_at,avatar) values(?,?,?,?,?)', [ loginname, email, pass, createAt, avatarUrl ], function(err, info) {
       callback(err, info);
    });
};
...
exports.updatePwd = function(userId, pwd, callback){
    mysql.update('update userBlog set pwd = ? where id = ?', [ common.md5(pwd), userId ], function(err,info){
        callback(err, info);
    });
};
