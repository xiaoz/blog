...
exports.queryCategory = function(categoryId, callback){
    mysql.queryOne('select * from product_categorys where id =?', [categoryId ], function(err, category) {
...
exports.deleteCategory = function(userId, categoryId, callback){
    mysql.update('delete from product_categorys where id=? and user_id=?', [ categoryId, userId ], function(err, info) {
        callback(err, info);
    });
};
...
exports.queryCategoriesOfUser = function(userId, callback) {
    mysql.query('select * from product_categorys where user_id = ? order by sequence asc', [ userId ], function(err, categories) {
        callback(err, categories);
    });
};
...
exports.queryArticlesCountOfCategories = function(categoryIds, callback){
    mysql.query('select category_id,count(product_id) as count from product_category where category_id in(' + categoryIds.join(',') + ') group by category_id', [], function(err, kvs) {
        callback(err, kvs);
    });
};
...
exports.updateCategory = function(name, sequence, userId, categoryId, callback) {
    mysql.update('update product_categorys set name=?, sequence=? where user_id = ? and id = ?', [ name, sequence, userId, categoryId ], function(err, info) {
        callback(err, info);
    });
};
...
exports.saveCategory = function(name, sequence, userId, callback) {
    mysql.update('insert into product_categorys(name,sequence,user_id) values(?,?,?)', [ name, sequence, userId ], function(err, info) {
        callback(err, info);
    });
};
